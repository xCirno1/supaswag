...
export const getPatientAnalysis = async (req: Request, res: Response) => {
  ...
  if (cached) { console.log(`[Cache HIT] ${cacheKey}`); return res.status(200).json(cached); }
...
export const getBulkInventoryNeeds = async (req: Request, res: Response) => {
  ...
  if (cached) { console.log(`[Cache HIT] ${CACHE_KEY_INVENTORY_NEEDS}`); return res.status(200).json(cached); }
...
export const getMealPlans = async (req: Request, res: Response) => {
  ...
  if (cached) { console.log(`[Cache HIT] ${CACHE_KEY_MEAL_PLANS}`); return res.status(200).json(cached); }
  ...
  try {
    ...
    const mealPlans = analyses.map(({
      patient, flaggedFoods, bmiCategory, targetCalories, mealPlanReason,
      suggestedBreakfast, suggestedBreakfastKcal,
      suggestedLunch, suggestedLunchKcal,
      suggestedDinner, suggestedDinnerKcal,
      suggestedSide,
    }) => ({
